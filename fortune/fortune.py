...
def fortune_files_from_paths(fortunepaths, offensive=None):
    """ Return (percentages, fortune_files)
        ...
        An offensive fortune fortune files is defined as who's name ends
        in '-o'
    """
...
def filter_fortunes(fortunepaths, pattern, ignorecase=True, offensive=None,
    ...
    """ Print out all fortunes which match the regular expression pattern.
        ...
        The fortunes are printed to standard output, while the names of the
        ...
        to a file, the result is a valid fortunes database file. If standard
        error is also redirected to this file, the result is still valid,
        but there will be ''bogus'' fortunes, i.e. the filenames themselves,
        in parentheses.
    ...
    while not found_first_match:
        ...
        while not found_first_match:
            try:
                ...
                break
    for start, length, fortune in fortunes:
        # remaining fortunes of "first" file
        if (length < min_length
                or (max_length is not None and length > max_length)):
            continue
        if regex_filter.search(fortune):
...
def main():
    ...
    arg_parser.add_option(
        '-w', '--wait', action='store', type=int, dest='seconds_to_wait',
    ...
    if len(args) >= 2:
        ...
        try:
            ...
            print >> sys.stderr, "Try %s --help" % os.path.basename(sys.argv[
                                                                    0])
            sys.exit(1)
